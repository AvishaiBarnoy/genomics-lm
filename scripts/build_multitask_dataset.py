...
def build_multitask_dataset():
    ...
    for ncbi_id, seq_info in protein_seqs.items():
        ...
        pfam = (
            str(pfam_raw).split(";")[0].strip()
            if pd.notna(pfam_raw) and pfam_raw
            else None
        )
    ...
    # Keep top 2000 Pfams
    top_pfams = df_samples["pfam"].value_counts().head(2000).index.tolist()
    pfam_vocab = {p: i for i, p in enumerate(top_pfams)}

    # Keep top 1000 ECs
    top_ecs = df_samples["ec"].value_counts().head(1000).index.tolist()
    ...
    # Save vocabularies
    vocabs = {"pfam": pfam_vocab, "ec": ec_vocab, "stability": stability_vocab}
        ...
        final_samples.append(
            {
                ...
                "stability_id": stability_vocab.get(s["stability"], -1)
                if s["stability"]
                else -1,
            }
        )

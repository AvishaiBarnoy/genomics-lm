...
def get_theoretical_shape(dna_seq):
    ...
    for i in range(len(dna_seq)):
        ...
        if "AAAA" in window: m_val = 3.5
        elif "GGGG" in window or "CCCC" in window: m_val = 5.8
        else: m_val = 4.5
        
        # Roll (Bendability/Step Rotation)
        if "GC" in window or "CG" in window: r_val = 5.0
        elif "AA" in window or "TT" in window: r_val = 0.0
        else: r_val = 2.5
        
        # EP (Electrostatic Potential)
        if "AAAA" in window: e_val = -10.0
        elif "GGCC" in window: e_val = -2.0
        else: e_val = -5.0

        # ProT (Propeller Twist) - Stiffer base pairing
        if "GC" in window: pr_val = -11.0
        elif "AT" in window: pr_val = -18.0
        else: pr_val = -14.0

        # HelT (Helix Twist) - Degrees per bp
        if "CG" in window: h_val = 36.0
        elif "TA" in window: h_val = 32.0
        else: h_val = 34.0
...
def probe_structural_awareness(run_id, ckpt="best.pt"):
    ...
    # 2. Get Hidden States
    with torch.no_grad():
        x = model.tok_emb(input_ids) + model.pos_emb(torch.arange(input_ids.size(1)).unsqueeze(0))

...
def main():
    ...
    ap.add_argument("--data_npz", required=True, help="Path to data npz containing 'X' (token IDs)")
    ap.add_argument("--n_samples", type=int, default=100, help="Number of sequences to process")
    ap.add_argument("--window_size", type=int, default=9, help="Sliding window size (in codons)")
    ap.add_argument("--stride", type=int, default=3, help="Sliding window stride")
    ap.add_argument("--n_clusters", type=int, default=10, help="Number of clusters (or min_cluster_size for HDBSCAN)")
    ap.add_argument("--method", default="kmeans", choices=["kmeans", "hdbscan"], help="Clustering algorithm")
    ap.add_argument("--pca", type=int, default=50, help="Number of PCA components before clustering")
    ap.add_argument("--layer", type=int, default=-1, help="Layer index to extract from (-1 is last)")
    args = ap.parse_args()
    ...
    print(f"[*] Extracting embeddings for {len(X)} sequences (window={args.window_size}, stride={args.stride})...")
    
    # Identify special token IDs to exclude from windows
    tokens = load_token_list(run_dir)
    tok_stoi = {t: i for i, t in enumerate(tokens)}
    special_ids = [tok_stoi[s] for s in ["<pad>", "<BOS_CDS>", "<EOS_CDS>", "<SEP>"] if s in tok_stoi]
    # Also check legacy/aliased specials
    for s in ["<bos>", "<eos>", "<eog>", "<PAD>"]:
        if s in tok_stoi: special_ids.append(tok_stoi[s])
    ...
    for i in range(len(X)):
        ...
        emb, metadata = extractor.extract(X[i:i+1], layer_idx=args.layer, exclude_ids=special_ids)
        if len(emb) > 0:
    ...
    if not all_embeddings:
        print("[!] No windows kept after filtering special tokens. Try a smaller window or check data.")
        return
    ...
    # Clustering
    print(f"[*] Clustering using {args.method} (n_clusters={args.n_clusters}, pca={args.pca})...")

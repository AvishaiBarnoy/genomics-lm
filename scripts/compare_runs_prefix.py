...
def main() -> None:
    ...
    ap.add_argument(
        "--metrics",
        default="median_gqs,mean_aa_identity",
    ...
    ks = (
        sorted(
            set.intersection(
                *[set(r["k"] for r in rows) for rows in summaries.values()]
            )
        )
        if summaries
        else []
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(out_dir / "compare_runs_prefix.png")
    plt.close(fig)
    print(
        f"[compare-prefix] wrote {combine_csv} and {out_dir / 'compare_runs_prefix.png'}"
    )

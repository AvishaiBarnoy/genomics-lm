...
from scripts._shared import load_model as load_codon_model
...
def main():
    ...
    ap.add_argument("--protein_ckpt", required=True, help="Path to ProteinLM checkpoint")
    ap.add_argument("--protein_cfg", required=True, help="Path to ProteinLM config")
    ap.add_argument("--conditions", nargs="*", help="Optional conditioning tokens, e.g. <FUNC:ENZYME>")
    args = ap.parse_args()

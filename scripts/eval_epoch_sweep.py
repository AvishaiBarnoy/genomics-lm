...
def main():
    ...
    ap.add_argument(
        "--max_windows", type=int, default=200, help="Number of windows for sanity KPIs"
    )
    ap.add_argument(
        "--keep_all",
        action="store_true",
        help="Keep all epoch checkpoints (do not delete)",
    )
    args = ap.parse_args()
    ...
    for ep_file in epoch_files:
        ...
        kpis = compute_kpis(
            model, device, test_npz, itos, sample_windows=args.max_windows
        )
        ...
        print(
            f"  Codon Corr: {kpis['codon_corr']:.4f} | Syn Gap: {kpis['syn_gap']:.4f} | Frameshift: {kpis['frameshift_delta']:.4f}"
        )
    ...
    metrics_to_rank = [
        ...
        "syn_gap",
        "frameshift_delta",
        "startstop_delta.start",
        "startstop_delta.stop",
    ]
    ...
    if not args.keep_all:
        ...
        for res in sweep_results:
            ...
            if p.exists() and res["epoch"] != best_epoch_idx:
                ...
                deleted_count += 1
        print(
            f"[cleanup] Deleted {deleted_count} epoch checkpoints. Kept epoch_{best_epoch_idx}.pt (as best_biological.pt)."
        )
    else:

...
def kl_div(p: Dict[str, float], q: Dict[str, float]) -> float:
    eps = 1e-12
    return float(
        sum(p[c] * np.log((p[c] + eps) / (q.get(c, 0.0) + eps)) for c in CODONS)
    )
...
def minhash_signature(
    seqs: Iterable[str], k: int, n_hash: int, seed: int = 1337
) -> np.ndarray:
    ...
    for s in seqs:
        ...
        for hidx, salt in enumerate(salts):
            for kmer in kmers:
                ...
                hv = np.uint64(
                    abs(hash(kmer)) ^ salt
                )  # heuristic; acceptable for relative comparison
                if hv < sig[hidx]:
                    sig[hidx] = hv
    return sig
...
def main() -> None:
    ...
    if args.run_id and not args.dna:
        ...
        out_dir = (
            Path(args.out_dir)
            if args.out_dir
            else (repo / "outputs" / "analysis" / "seq_quality")
        )
    ...
    with summary_csv.open("w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=list(rows[0].keys()) if rows else ["id", "len_codon"]
        )
        writer.writeheader()
    ...
    if lengths:
        ...
        plt.hist(lengths, bins=30, alpha=0.85)
        plt.xlabel("length (codons)")
        plt.ylabel("count")
        plt.tight_layout()
    ...
    if gcs:
        plt.figure(figsize=(5, 3))
        plt.hist(gcs, bins=30, alpha=0.85)
        plt.xlabel("GC%")
        plt.ylabel("count")
        plt.tight_layout()
        plt.savefig(out_dir / "gc_hist.png")
        plt.close()
    if periodicities:
        plt.figure(figsize=(5, 3))
        plt.hist(periodicities, bins=30, alpha=0.85)
        plt.xlabel("FFT power at 1/3")
        plt.ylabel("count")
        plt.tight_layout()
        plt.savefig(out_dir / "periodicity.png")
        plt.close()
    ...
    if args.run_id:
        ...
        metrics.update(
            {
                "orf_ok_rate": float(np.mean([r["orf_ok"] for r in rows]))
                if rows
                else float("nan"),
                "len_codon_median": float(np.median(lengths))
                if lengths
                else float("nan"),
                "gc_percent_median": float(np.median(gcs)) if gcs else float("nan"),
                "fft_1over3_median": float(np.median(periodicities))
                if periodicities
                else float("nan"),

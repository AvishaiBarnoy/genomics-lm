...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=200,
        help="number of optimizer steps to record (after warmup)",
    )
    args = ap.parse_args()
    ...
    def on_trace_ready(p):
        ...
        with txt_path.open("w") as fh:
            fh.write(
                p.key_averages().table(sort_by="self_cpu_time_total", row_limit=200)
            )
    ...
    print(
        "Open the trace in Chrome: chrome://tracing (Load trace) or VS Code Torch-Profiler viewer."
    )

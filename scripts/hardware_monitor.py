...
def monitor():
    ...
    while True:
        ...
        current_size = (
            os.path.getsize(CURVES_PATH) if os.path.exists(CURVES_PATH) else 0
        )

        # Check for progress
        if current_size > last_size:
            print(
                f"[Heartbeat] Step completed. Current RAM: {current_ram} GB. Time since last: {time.time() - start_time:.2f}s"
            )

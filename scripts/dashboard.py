...
def main():
    ...
    parser.add_argument(
        "--runs", type=str, help="Comma-separated list of run IDs to compare"
    )
    parser.add_argument(
        "--scores_dir",
        type=str,
        default="outputs/scores",
        help="Base directory for scores",
    )
    parser.add_argument(
        "--runs_dir", type=str, default="runs", help="Base directory for run artifacts"
    )
    parser.add_argument(
        "--export", action="store_true", help="Export a full Markdown report with plots"
    )
    parser.add_argument(
        "--out_dir", type=str, help="Directory to save the exported report"
    )
    ...
    aggregator = ResultsAggregator(
        run_ids=run_ids, scores_base_dir=args.scores_dir, runs_base_dir=args.runs_dir
    )

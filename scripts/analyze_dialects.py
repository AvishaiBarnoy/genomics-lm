...
def analyze_dialects():
    ...
    # Identify groups
    gram_pos_ids = [
        "_".join(Path(f).stem.split("_")[:2])
        for f in Path("data/raw/gram_pos").glob("*.gbff")
    ]
    high_gc_ids = [
        "_".join(Path(f).stem.split("_")[:2])
        for f in Path("data/raw/high-gc").glob("*.gbff")
    ]

    def get_group(genome):
        if genome in gram_pos_ids:
            return "Gram-positive"
        if genome in high_gc_ids:
            return "High-GC"

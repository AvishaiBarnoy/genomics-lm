...
def audit_motifs(run_id):
    ...
    for m in motifs:
        ...
        features = []
        if m_stats["Roll"] > 4.0:
            features.append("Highly Flexible/Bent")
        elif m_stats["Roll"] < 1.0:
            features.append("Stiff/Straight")

        if m_stats["MGW"] < 3.8:
            features.append("Narrow Minor Groove")
        elif m_stats["MGW"] > 5.2:
            features.append("Wide Minor Groove")

        if m_stats["EP"] < -8.0:
            features.append("High Negative Charge (Attractive)")
        ...
        print(
            f"  Biological Role: {', '.join(features) if features else 'Standard B-DNA'}"
        )

...
def generate_report(run_id: str):
    ...
    report.append("This AI model was trained on DNA sequences without any prior knowledge of biology. "
    ...
    report.append("We performed a 'stress test' on a gene (e.g., lacZ) by changing every single DNA letter "
    ...
    report.append("The following positions in the gene are identified as **Critical Zones**. "
                  "The AI believes that almost any change here would make the sequence 'unnatural' or 'wrong':")
    for _, row in sensitive_df.iterrows():
        report.append(f"- **Position {int(row['pos'])}** (Current Codon: `{row['wt']}`): Very high sensitivity. No substitutions were predicted to be better.")
    report.append("")

    # Section 2: Flexible Sites
    report.append("## 🟢 Flexible 'Adaptable' Sites")
    report.append("Conversely, some areas are highly flexible. The AI predicts that many different DNA sequences "
    ...
    for _, row in flexible_df.iterrows():
        report.append(f"- **Position {int(row['pos'])}**: Highly flexible. There are {int(row['n_better'])} alternative DNA sequences that the model likes.")
    report.append("")

    # Section 3: Discovered Motifs
    report.append("## 🧩 Discovered Biological Motifs")
    report.append("The AI has learned that certain patterns of DNA letters appear together frequently in meaningful ways. "
                  "We call these 'Motifs'. Here are the top patterns the AI looks for:")
    
    for m in motifs[:5]:
        report.append(f"- **{m['name']}**: Consistently finds patterns like `{m['consensus']}`.")
    report.append("")

    # Section 4: What this means for drug design/bioengineering
    report.append("## 🚀 Why this matters")
    report.append("1. **Gene Engineering**: We can use the 'Green Zones' to optimize DNA sequences for better manufacturing without breaking the biology.")
    report.append("2. **Mutation Prediction**: The 'Red Zones' tell us where genetic diseases are most likely to occur if a mutation happens.")
    report.append("3. **De-novo Design**: Because the AI understands these patterns, we can eventually ask it to 'write' new genes that follow these rules.")

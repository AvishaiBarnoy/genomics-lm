...
def audit_dataset():
    ...
    gram_pos_files = [Path(f).stem.split("_")[:2] for f in Path("data/raw/gram_pos").glob("*.gbff")]
    gram_pos_ids = ["_".join(p) for p in gram_pos_files]
    
    high_gc_files = [Path(f).stem.split("_")[:2] for f in Path("data/raw/high-gc").glob("*.gbff")]

...
*   **Data Packing & Masking:**
    ...
    *   Lossless chunking: `block_size` is the number of model transitions, so prepared chunks contain at most `block_size + 1` tokens. Long fragments use deterministic one-token overlap: BOS occurs only in the first chunk, EOS only in the last, and every source next-token transition occurs exactly once. Global and legacy builders ignore `windows_per_seq` replication and emit each transition once.

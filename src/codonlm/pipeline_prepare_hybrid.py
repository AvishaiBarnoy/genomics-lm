#!/usr/bin/env python3
"""
Prepare hybrid datasets for the training pipeline.
Handles extraction, tokenization, dataset splitting, stacking, and integrity checks.
"""
...
def _make_dataset_entry(entry: Dict[str, object], block_size: int) -> Dict[str, str]:
    ...
    out_dir = Path("data/processed") / (name + "_hybrid")
    ...
    return {
        ...
        "min_len": min_len,
        "out_dir": str(out_dir),
        "tsv": str(out_dir / "hybrid_data.tsv"),
        "meta": str(out_dir / "hybrid_meta.tsv"),
        "ids": str(out_dir / "hybrid_ids.txt"),
        "vocab": str(out_dir / "vocab_hybrid.txt"),
        "itos": str(out_dir / "itos_hybrid.txt"),
...
def _ensure_dataset(
    ...
    cmds: List[List[str]] = []
    if force or not (tsv.exists() and meta.exists()):
        cmds.append(
            [
                "python",
                "-m",
                "src.codonlm.extract_hybrid_from_genbank",
                "--gbff",
                entry["gbff"],
                "--out_tsv",
                str(tsv),
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Prepare hybrid datasets for training pipeline"
    )
    ...
    has_ids = []
    itos_sets = []
    expected_specials = ["<PAD>", "<BOS_CDS>", "<EOS_CDS>", "<UNK>", "<UTR_START>", "<UTR_END>"]
    for ds in datasets:
        ids_p = Path(ds["ids"])
        itos_p = Path(ds["itos"])
        ...
        if itos_p.exists():
            try:
                ...
                itos_sets.append(toks[:6])
            except Exception:
                pass
    mixed_state = bool(needs_ids) and bool(has_ids)
    bad_specials = False
    for toks in itos_sets:
        if len(toks) < 6 or toks[:6] != expected_specials:
            bad_specials = True
            break
    ...
    combined_dir = Path("data/processed/combined_hybrid") / args.run_id
